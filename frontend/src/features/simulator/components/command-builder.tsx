import { useState, useEffect, useMemo, useRef } from "react";
import { HugeiconsIcon } from "@hugeicons/react";
import {
  PlayIcon,
  Add01Icon,
  Delete01Icon,
  HelpCircleIcon,
  ArrowDown01Icon,
  ArrowRight01Icon,
  InformationCircleIcon,
} from "@hugeicons/core-free-icons";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Badge } from "@/components/ui/badge";
import { Spinner } from "@/components/ui/spinner";
import {
  Card,
  CardContent,
  CardDescription,
  CardFooter,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import {
  Field,
  FieldGroup,
  FieldLabel,
  FieldDescription,
} from "@/components/ui/field";
import {
  Select,
  SelectContent,
  SelectGroup,
  SelectItem,
  SelectLabel,
  SelectSeparator,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select";
import { Skeleton } from "@/components/ui/skeleton";
import { Separator } from "@/components/ui/separator";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
  TooltipTrigger,
} from "@/components/ui/tooltip";
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
} from "@/components/ui/collapsible";
import { cn, truncateId, formatTemplateId, partitionPackages } from "@/lib/utils";
import { useQuery } from "@tanstack/react-query";
import { api } from "@/lib/api";
import type {
  SimulationRequest,
  SimulationCommand,
  DisclosedContract,
  TemplateDefinition,
  TemplateId,
} from "@/lib/types";
import { ArgumentForm } from "./argument-form";
import { useContractAutocomplete } from "../hooks";

// ---------------------------------------------------------------------------
// Disclosed contract entry
// ---------------------------------------------------------------------------

interface DisclosedEntry {
  id: string;
  contractId: string;
  templatePackage: string;
  templateModule: string;
  templateEntity: string;
  eventBlob: string;
}

// ---------------------------------------------------------------------------
// Contract ID autocomplete component
// ---------------------------------------------------------------------------

function ContractIdAutocomplete({
  value,
  onChange,
}: {
  value: string;
  onChange: (value: string) => void;
}) {
  const [query, setQuery] = useState(value);
  const [open, setOpen] = useState(false);
  const inputRef = useRef<HTMLInputElement>(null);
  const { data: acsData } = useContractAutocomplete(query);
  const suggestions = acsData?.contracts ?? [];

  useEffect(() => {
    setQuery(value);
  }, [value]);

  const handleSelect = (contractId: string) => {
    onChange(contractId);
    setQuery(contractId);
    setOpen(false);
  };

  return (
    <div className="relative">
      <Input
        ref={inputRef}
        className="font-mono text-xs"
        placeholder="Start typing to search ACS..."
        value={query}
        onChange={(e) => {
          setQuery(e.target.value);
          onChange(e.target.value);
          setOpen(true);
        }}
        onFocus={() => {
          if (query.length >= 2) setOpen(true);
        }}
        onBlur={() => {
          setTimeout(() => setOpen(false), 200);
        }}
      />
      {open && suggestions.length > 0 && (
        <div className="absolute top-full z-50 mt-1 max-h-48 w-full overflow-y-auto rounded-xl border bg-popover shadow-lg">
          {suggestions.map((c) => (
            <button
              key={c.contractId}
              className="flex w-full items-center gap-2 px-3 py-2 text-left text-xs hover:bg-accent"
              onMouseDown={(e) => {
                e.preventDefault();
                handleSelect(c.contractId);
              }}
            >
              <span className="truncate font-mono">
                {truncateId(c.contractId, 20)}
              </span>
              <Badge
                variant="outline"
                className="ml-auto flex-shrink-0 text-[9px]"
              >
                {formatTemplateId(c.templateId)}
              </Badge>
            </button>
          ))}
        </div>
      )}
    </div>
  );
}

// ---------------------------------------------------------------------------
// Command builder component
// ---------------------------------------------------------------------------

export interface CommandBuilderProps {
  onSimulate: (request: SimulationRequest) => void;
  isSimulating: boolean;
  initialValues?: Partial<{
    packageId: string;
    template: string;
    choice: string;
    contractId: string;
    args: Record<string, unknown>;
    actAs: string;
    readAs: string;
    mode: "online" | "offline";
    offset: string;
  }>;
}

export function CommandBuilder({
  onSimulate,
  isSimulating,
  initialValues,
}: CommandBuilderProps) {
  // Parse initial template value — may be "ModuleName:EntityName" from URL
  const parsedInitialTemplate = useMemo(() => {
    const raw = initialValues?.template ?? "";
    if (raw.includes(":")) {
      return raw.split(":").pop() ?? "";
    }
    return raw;
  }, [initialValues?.template]);

  // Form state
  const [selectedPackageId, setSelectedPackageId] = useState(
    initialValues?.packageId ?? ""
  );
  const [selectedTemplate, setSelectedTemplate] = useState(
    parsedInitialTemplate
  );
  const [selectedChoice, setSelectedChoice] = useState(
    initialValues?.choice ?? ""
  );
  const [contractId, setContractId] = useState(
    initialValues?.contractId ?? ""
  );
  const [args, setArgs] = useState<Record<string, unknown>>(
    initialValues?.args ?? {}
  );
  const [actingParties, setActingParties] = useState(
    initialValues?.actAs ?? ""
  );
  const [readAsParties, setReadAsParties] = useState(
    initialValues?.readAs ?? ""
  );
  const [mode, setMode] = useState<"online" | "offline">(
    initialValues?.mode ?? "online"
  );
  const [historicalOffset, setHistoricalOffset] = useState(
    initialValues?.offset ?? ""
  );
  const [disclosedEntries, setDisclosedEntries] = useState<DisclosedEntry[]>(
    []
  );
  const [disclosedOpen, setDisclosedOpen] = useState(false);

  // Fetch packages
  const { data: packages, isLoading: packagesLoading } = useQuery({
    queryKey: ["packages-summary"],
    queryFn: () => api.getPackages().then((r) => r.data),
    staleTime: 5 * 60 * 1000,
  });

  // Partition packages into user vs system
  const [userPackages, systemPackages] = useMemo(
    () => partitionPackages(packages ?? []),
    [packages]
  );

  // Auto-select the first user package that contains the initial template.
  // This fires once when packages finish loading and an initial template is provided
  // via URL query params (e.g. from ACS Inspector "Use in Simulation").
  useEffect(() => {
    if (!parsedInitialTemplate || selectedPackageId || !packages) return;
    // Prefer user packages, fall back to system packages
    const candidates = [...userPackages, ...systemPackages];
    if (candidates.length > 0) {
      // We don't know which package contains the template until we fetch its
      // detail, so select the first user package as a best guess.
      setSelectedPackageId(candidates[0].packageId);
    }
  }, [packages, parsedInitialTemplate, selectedPackageId, userPackages, systemPackages]);

  // Fetch package detail
  const { data: packageDetail, isLoading: detailLoading } = useQuery({
    queryKey: ["package-detail", selectedPackageId],
    queryFn: () =>
      api.getPackageTemplates(selectedPackageId).then((r) => r.data),
    enabled: !!selectedPackageId,
    staleTime: 10 * 60 * 1000,
  });

  const allTemplates: TemplateDefinition[] =
    packageDetail?.modules.flatMap((m) => m.templates) ?? [];
  const currentTemplate = allTemplates.find(
    (t) => t.name === selectedTemplate
  );
  const currentChoice = currentTemplate?.choices.find(
    (c) => c.name === selectedChoice
  );

  // Reset downstream when parent changes
  useEffect(() => {
    if (!initialValues?.template) {
      setSelectedTemplate("");
      setSelectedChoice("");
      setArgs({});
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedPackageId]);

  useEffect(() => {
    if (!initialValues?.choice) {
      setSelectedChoice("");
      setArgs({});
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedTemplate]);

  useEffect(() => {
    if (!initialValues?.args) {
      setArgs({});
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedChoice]);

  // Disclosed contract helpers
  const addDisclosed = () => {
    setDisclosedEntries((prev) => [
      ...prev,
      {
        id: crypto.randomUUID(),
        contractId: "",
        templatePackage: "",
        templateModule: "",
        templateEntity: "",
        eventBlob: "",
      },
    ]);
    setDisclosedOpen(true);
  };

  const removeDisclosed = (id: string) => {
    setDisclosedEntries((prev) => prev.filter((e) => e.id !== id));
  };

  const updateDisclosed = (
    id: string,
    field: keyof DisclosedEntry,
    value: string
  ) => {
    setDisclosedEntries((prev) =>
      prev.map((e) => (e.id === id ? { ...e, [field]: value } : e))
    );
  };

  // Submit
  const handleSimulate = () => {
    if (!currentTemplate) return;

    const module = packageDetail?.modules.find((m) =>
      m.templates.some((t) => t.name === selectedTemplate)
    );

    const templateId: TemplateId = {
      packageName: packageDetail?.packageName ?? selectedPackageId,
      moduleName: module?.name ?? "",
      entityName: selectedTemplate,
    };

    const command: SimulationCommand = {
      templateId,
      choice: selectedChoice || undefined,
      contractId: contractId || undefined,
      arguments: args,
    };

    const disclosed: DisclosedContract[] = disclosedEntries
      .filter((e) => e.contractId)
      .map((e) => ({
        contractId: e.contractId,
        templateId: {
          packageName: e.templatePackage,
          moduleName: e.templateModule,
          entityName: e.templateEntity,
        },
        payload: {},
        createdEventBlob: e.eventBlob || undefined,
      }));

    const request: SimulationRequest = {
      mode,
      commands: [command],
      actAs: actingParties
        .split(",")
        .map((p) => p.trim())
        .filter(Boolean),
      readAs: readAsParties
        .split(",")
        .map((p) => p.trim())
        .filter(Boolean),
      disclosedContracts: disclosed.length > 0 ? disclosed : undefined,
      historicalOffset: historicalOffset || undefined,
    };

    onSimulate(request);
  };

  return (
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle>Command Builder</CardTitle>
            <CardDescription>
              Configure the Daml command to simulate
            </CardDescription>
          </div>
          {/* Simulation mode pill toggle in header */}
          <div className="flex items-center gap-1.5">
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <HugeiconsIcon
                    icon={HelpCircleIcon}
                    className="size-3.5 text-muted-foreground"
                    strokeWidth={2}
                  />
                </TooltipTrigger>
                <TooltipContent className="max-w-xs">
                  <p className="text-xs">
                    <strong>Online (Prepare):</strong> Uses participant's
                    Prepare endpoint (requires CanActAs).
                  </p>
                  <p className="mt-1 text-xs">
                    <strong>Offline (Engine):</strong> Uses our engine
                    (requires CanReadAs only).
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
            <div className="flex items-center rounded-full bg-muted p-1">
              <button
                className={cn(
                  "rounded-full px-3 py-1 text-xs font-medium transition-colors",
                  mode === "online"
                    ? "bg-background text-foreground shadow-sm"
                    : "text-muted-foreground hover:text-foreground"
                )}
                onClick={() => setMode("online")}
              >
                Online
              </button>
              <button
                className={cn(
                  "rounded-full px-3 py-1 text-xs font-medium transition-colors",
                  mode === "offline"
                    ? "bg-background text-foreground shadow-sm"
                    : "text-muted-foreground hover:text-foreground"
                )}
                onClick={() => setMode("offline")}
              >
                Offline
              </button>
            </div>
          </div>
        </div>
      </CardHeader>

      <CardContent>
        <FieldGroup>
          {/* Command selection row: Package, Template, Choice */}
          <div className="grid grid-cols-3 gap-3">
            <Field>
              <FieldLabel>Package</FieldLabel>
              {packagesLoading ? (
                <Skeleton className="h-9 w-full" />
              ) : (
                <Select
                  value={selectedPackageId}
                  onValueChange={setSelectedPackageId}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select..." />
                  </SelectTrigger>
                  <SelectContent>
                    {userPackages.length > 0 && (
                      <SelectGroup>
                        <SelectLabel className="text-[10px] text-muted-foreground">
                          User Packages
                        </SelectLabel>
                        {userPackages.map((pkg) => (
                          <SelectItem
                            key={pkg.packageId}
                            value={pkg.packageId}
                          >
                            <span className="text-xs">
                              {pkg.packageName ?? pkg.packageId.slice(0, 16)}
                              {pkg.packageVersion &&
                                ` v${pkg.packageVersion}`}
                            </span>
                          </SelectItem>
                        ))}
                      </SelectGroup>
                    )}
                    {systemPackages.length > 0 && (
                      <>
                        {userPackages.length > 0 && <SelectSeparator />}
                        <SelectGroup>
                          <SelectLabel className="text-[10px] text-muted-foreground">
                            System Packages
                          </SelectLabel>
                          {systemPackages.map((pkg) => (
                            <SelectItem
                              key={pkg.packageId}
                              value={pkg.packageId}
                              className="text-muted-foreground"
                            >
                              <span className="text-xs">
                                {pkg.packageName ??
                                  pkg.packageId.slice(0, 16)}
                                {pkg.packageVersion &&
                                  ` v${pkg.packageVersion}`}
                              </span>
                            </SelectItem>
                          ))}
                        </SelectGroup>
                      </>
                    )}
                  </SelectContent>
                </Select>
              )}
            </Field>

            <Field>
              <FieldLabel>Template</FieldLabel>
              {detailLoading ? (
                <Skeleton className="h-9 w-full" />
              ) : (
                <Select
                  value={selectedTemplate}
                  onValueChange={setSelectedTemplate}
                  disabled={
                    !selectedPackageId || allTemplates.length === 0
                  }
                >
                  <SelectTrigger>
                    <SelectValue
                      placeholder={
                        selectedPackageId &&
                        !detailLoading &&
                        allTemplates.length === 0
                          ? "No templates"
                          : "Select..."
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
                    {allTemplates.map((t) => (
                      <SelectItem key={t.name} value={t.name}>
                        <span className="text-xs">{t.name}</span>
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              )}
            </Field>

            <Field>
              <FieldLabel>Choice</FieldLabel>
              <Select
                value={selectedChoice}
                onValueChange={setSelectedChoice}
                disabled={!currentTemplate}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Select..." />
                </SelectTrigger>
                <SelectContent>
                  {currentTemplate?.choices.map((c) => (
                    <SelectItem key={c.name} value={c.name}>
                      <span className="flex items-center gap-2 text-xs">
                        {c.name}
                        {c.consuming && (
                          <Badge
                            variant="outline"
                            className="text-[9px] text-destructive"
                          >
                            consuming
                          </Badge>
                        )}
                      </span>
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </Field>
          </div>

          {/* Contract ID - shown when a choice is selected */}
          {selectedChoice && (
            <Field>
              <FieldLabel>Contract ID</FieldLabel>
              <ContractIdAutocomplete
                value={contractId}
                onChange={setContractId}
              />
              <FieldDescription>
                Required for exercise commands. Start typing to search the
                active contract set.
              </FieldDescription>
            </Field>
          )}

          {/* Parties row */}
          <div className="grid grid-cols-2 gap-3">
            <Field>
              <FieldLabel>Acting As</FieldLabel>
              <Input
                className="font-mono text-xs"
                placeholder="party1, party2..."
                value={actingParties}
                onChange={(e) => setActingParties(e.target.value)}
              />
            </Field>
            <Field>
              <FieldLabel>Read As</FieldLabel>
              <Input
                className="font-mono text-xs"
                placeholder="party1, party2..."
                value={readAsParties}
                onChange={(e) => setReadAsParties(e.target.value)}
              />
            </Field>
          </div>

          {/* Historical offset (only for offline mode) */}
          {mode === "offline" && (
            <Field>
              <FieldLabel>Historical Offset</FieldLabel>
              <Input
                className="font-mono text-xs"
                placeholder="Leave blank for current ledger end"
                value={historicalOffset}
                onChange={(e) => setHistoricalOffset(e.target.value)}
              />
              <FieldDescription>
                Optional. Simulate against a historical ledger state.
              </FieldDescription>
            </Field>
          )}

          {/* Dynamic choice arguments */}
          {currentChoice && (
            <>
              <Separator />
              <Field>
                <FieldLabel>Choice Arguments</FieldLabel>
                <div className="rounded-2xl border bg-muted/30 p-4">
                  <ArgumentForm
                    parameters={currentChoice.parameters}
                    values={args}
                    onChange={setArgs}
                  />
                </div>
              </Field>
            </>
          )}

          {/* Disclosed contracts - collapsible */}
          <Separator />
          <Collapsible open={disclosedOpen} onOpenChange={setDisclosedOpen}>
            <div className="flex items-center justify-between">
              <CollapsibleTrigger className="flex items-center gap-2 text-sm font-medium hover:text-foreground">
                {disclosedOpen ? (
                  <HugeiconsIcon
                    icon={ArrowDown01Icon}
                    className="size-4"
                    strokeWidth={2}
                  />
                ) : (
                  <HugeiconsIcon
                    icon={ArrowRight01Icon}
                    className="size-4"
                    strokeWidth={2}
                  />
                )}
                Disclosed Contracts
                {disclosedEntries.length > 0 && (
                  <Badge variant="secondary" className="text-[10px]">
                    {disclosedEntries.length}
                  </Badge>
                )}
              </CollapsibleTrigger>
              <Button size="xs" variant="outline" onClick={addDisclosed}>
                <HugeiconsIcon
                  icon={Add01Icon}
                  data-icon="inline-start"
                  strokeWidth={2}
                />
                Add
              </Button>
            </div>

            <CollapsibleContent>
              {disclosedEntries.length === 0 ? (
                <p className="mt-3 text-sm text-muted-foreground">
                  No disclosed contracts added. These are only needed for
                  cross-domain simulation scenarios.
                </p>
              ) : (
                <div className="mt-3 flex flex-col gap-3">
                  {disclosedEntries.map((entry) => (
                    <div
                      key={entry.id}
                      className="flex flex-col gap-3 rounded-2xl border bg-muted/20 p-4"
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium">
                          Disclosed Contract
                        </span>
                        <Button
                          size="icon-xs"
                          variant="ghost"
                          onClick={() => removeDisclosed(entry.id)}
                        >
                          <HugeiconsIcon
                            icon={Delete01Icon}
                            className="text-muted-foreground"
                            strokeWidth={2}
                          />
                        </Button>
                      </div>
                      <Input
                        className="font-mono text-xs"
                        placeholder="Contract ID"
                        value={entry.contractId}
                        onChange={(e) =>
                          updateDisclosed(
                            entry.id,
                            "contractId",
                            e.target.value
                          )
                        }
                      />
                      <div className="grid grid-cols-3 gap-2">
                        <Input
                          className="font-mono text-xs"
                          placeholder="Package name"
                          value={entry.templatePackage}
                          onChange={(e) =>
                            updateDisclosed(
                              entry.id,
                              "templatePackage",
                              e.target.value
                            )
                          }
                        />
                        <Input
                          className="font-mono text-xs"
                          placeholder="Module name"
                          value={entry.templateModule}
                          onChange={(e) =>
                            updateDisclosed(
                              entry.id,
                              "templateModule",
                              e.target.value
                            )
                          }
                        />
                        <Input
                          className="font-mono text-xs"
                          placeholder="Entity name"
                          value={entry.templateEntity}
                          onChange={(e) =>
                            updateDisclosed(
                              entry.id,
                              "templateEntity",
                              e.target.value
                            )
                          }
                        />
                      </div>
                      <Input
                        className="font-mono text-xs"
                        placeholder="Created event blob (base64)"
                        value={entry.eventBlob}
                        onChange={(e) =>
                          updateDisclosed(
                            entry.id,
                            "eventBlob",
                            e.target.value
                          )
                        }
                      />
                    </div>
                  ))}
                </div>
              )}
            </CollapsibleContent>
          </Collapsible>
        </FieldGroup>
      </CardContent>

      <CardFooter className="flex flex-col gap-3">
        {/* Full-width Simulate button */}
        <Button
          className="w-full"
          size="lg"
          onClick={handleSimulate}
          disabled={!currentTemplate || isSimulating}
        >
          {isSimulating ? (
            <Spinner data-icon="inline-start" />
          ) : (
            <HugeiconsIcon
              icon={PlayIcon}
              data-icon="inline-start"
              strokeWidth={2}
            />
          )}
          Simulate
        </Button>

        {/* Subtle state drift note */}
        <div className="flex items-start gap-2">
          <HugeiconsIcon
            icon={InformationCircleIcon}
            className="mt-px size-3.5 flex-shrink-0 text-muted-foreground/60"
            strokeWidth={2}
          />
          <p className="text-xs text-muted-foreground/60">
            Results are predictions based on current state. Between simulation
            and execution, contracts may be archived and costs may differ.
          </p>
        </div>
      </CardFooter>
    </Card>
  );
}
